...
VARIANTS = [
    ("1-baseline",    "../networks/BELAGERE.net.xml",
     "Baseline — u-turns intact, unsignalised"),
    ("2-nouturn",     "../networks/balegere-no-uturn.net.xml",
     "U-turn ban, unsignalised"),
    ("3-nouturn-tls", "../networks/balegere-no-uturn-traffic.net.xml",
     "U-turn ban + original 171 s 4-phase signal"),
    ("4-retimed",     "nets/V4-retimed.net.xml",
     "T1 — retimed to 90 s 2-phase actuated, triangle kept, 1 lane"),
    ("5-joined",      "nets/V5-joined.net.xml",
     "T1+T2 — nodes joined into one 4-way signal, still 1 lane"),
    ("6-2lane",       "nets/V6-2lane.net.xml",
     "T1+T2+T3 — joined 4-way, 2 lanes every approach"),
    ("7-3lane-ew",    "nets/V7-3lane-ew.net.xml",
     "T1+T2+T3 — joined 4-way, 3 lanes E–W, 2 lanes N–S"),
    ("8-tri-wide",    "nets/V8-triangle-wide.net.xml",
     "T1+T3 without T2 — triangle kept but widened"),
    ("9-3lane-all",   "nets/V9-3lane-all.net.xml",
     "T1+T2+T3 — joined 4-way, 3 lanes every approach"),
]
...
if __name__ == "__main__":
    ...
    for tag, path, desc in VARIANTS:
        ...
        if tl:
            frag.append("<h4>Signal program</h4>")
            frag.append('<div class="diagram-embed-wrap" style="padding:14px 10px;">')
    ...
    with open(os.path.join(OUT, "variants.html"), "w") as f:
